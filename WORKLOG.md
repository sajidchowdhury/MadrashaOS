...
Task ID: B2
Agent: full-stack-developer
Task: Build the complete Authentication & RBAC system (B2.1 NextAuth + JWT/refresh + B2.2 permission middleware + B2.3 MFA TOTP + B2.4 verify-roles API + login page).

Work Log:
- Read worklog.md, schema.prisma (52-model), db.ts, permissions.ts (110+ codes), role-permissions.ts (8 roles), stores/types.ts (8 personas), .env, package.json. Confirmed Prisma Client generated at src/generated/prisma, bcryptjs already a dependency, next-auth@4.24.11 already installed.
- Installed `otplib@13.5.0` + `qrcode@1.5.4` + `@types/qrcode` for TOTP MFA (B2.3). Verified `jose` (transitive of next-auth) is present for standalone JWT signing.
- Created `src/lib/auth/password.ts` — bcrypt wrappers: `hashPassword` (10 rounds), `verifyPassword` (constant-time, never throws), `generateTempPassword` (12 chars from unambiguous alphabet, uses `node:crypto.randomInt` for unbiased randomness — not `Math.random` per SRS §6.2.3 R-S2).
- Created `src/lib/auth/tokens.ts` — standalone JWT helpers layered on `jose`: `generateAccessToken` (15-min TTL), `generateRefreshToken` (7-day TTL with family_id for rotation theft detection), `verifyToken` (returns null instead of throwing — uniform failure path), `rotateRefreshToken` (stateless rotation in same family). Constants `ACCESS_TOKEN_TTL_SECONDS=900`, `REFRESH_TOKEN_TTL_SECONDS=604800` exported for downstream consumers.
- Created `src/lib/auth/config.ts` — NextAuth v4 config: JWT strategy, CredentialsProvider with email+password, 15-min session maxAge, pages.signIn=/login, pages.signOut=/logout, NEXTAUTH_SECRET from env. authorize() flow: lookup user via Prisma (with role + permission select), lockout check (R-S1: 5 fails → 15-min lock), status check (R-S4), bcrypt verify, on-fail increment counter, on-success reset counter + update last_login_at, fetch permissions via role_permissions junction. MFA gate: if user.mfa_enabled → returns user with `mfa_pending=true` + empty permissions[] so the JWT is issued but downstream `withPermission` middleware rejects every API call until /api/v1/auth/mfa/verify succeeds. callbacks.jwt injects role + organization_id + branch_id + permissions + mfa_pending on sign-in; callbacks.session exposes them on session.user. runtime="nodejs" (Prisma needs libquery_engine).
- Created `src/types/next-auth.d.ts` — module augmentation for `next-auth` (Session + User interfaces) and `next-auth/jwt` (JWT interface) to expose the custom fields (role, organization_id, branch_id, permissions, mfa_pending) to all consumers of `useSession()` / `getSession()` / `getServerSession()`.
- Created `src/app/api/auth/[...nextauth]/route.ts` — NextAuth v4 route handler mounted at /api/auth/*. Exports GET + POST, runtime="nodejs", dynamic="force-dynamic".
- Created `src/middleware.ts` — Edge middleware using `getToken()` from `next-auth/jwt` directly (NOT `withAuth` — withAuth always returns 307 redirect for unauthenticated requests, which doesn't fit the JSON-API contract). Route rules: (1) `/api/auth/*` public; (2) `POST /api/v1/donations` public per SRS §6.5 Risk R10; (3) all other `/api/v1/*` require session — returns 401 JSON for API requests, 307 redirect to /login for browser navigations; (4) non-/api routes pass through. Matcher limits middleware to `/api/:path*` to keep static assets + public site fast.
- Created `src/app/api/v1/auth/session/route.ts` — GET session info. Returns `{ user: { id, name, email, role, organization_id, branch_id }, permissions, expires }` on 200; 401 JSON if no session; 403 with `error: "MFA_REQUIRED"` if session is mfa_pending (and the user's basic info so the client can render the MFA form).
- Created `src/lib/auth/with-permission.ts` — higher-order function `withPermission(code, handler)` returning a RequestHandler that: (a) fetches session via getServerSession; (b) returns 401 if unauthenticated; (c) returns 403 MFA_REQUIRED if mfa_pending; (d) returns 403 Forbidden if permissions[] doesn't include `code`. Also exports `withPermissions(codes[], handler)` (ALL codes required, AND semantics) and `withAnyPermission(codes[], handler)` (ANY one code required, OR semantics). Plus `getCurrentSession()` convenience helper.
- Created `src/lib/auth/with-tenant.ts` — multi-tenant scoping helpers: `getTenantContext()` returns `{ organization_id, branch_id, user_id, role, permissions }` from the current session (null if unauthenticated or mfa_pending). `hasPermission(code)` boolean check. `tenantWhere(ctx)` returns a Prisma where-fragment that scopes by organization_id (+ branch_id if non-null, except for super-admin and org-level authority roles which get a wider scope). Handlers spread this into their queries: `where: { ...tenantWhere(ctx), status: "active" }`.
- Created `src/lib/auth/with-idempotency.ts` — `withIdempotency(handler)` wrapper per SRS §6.5: checks `Idempotency-Key` header on POST/PUT/PATCH/DELETE (GET bypasses — naturally idempotent). In-memory Map cache (24h TTL, swap to Redis in prod). On cache hit → returns cached status + body + `X-Idempotent-Replay: true` header. On miss → runs handler, caches 2xx/4xx responses (NOT 5xx so client can retry with same key). Lazy TTL sweep on every miss.
- Created `src/lib/auth/with-audit.ts` — `withAudit(entityType, handler)` wrapper that runs the handler, captures the response body's `data.id` + `data` snapshot, and writes a row to `audit_logs` with: organization_id, branch_id, actor_user_id, action (POST→create / PUT,PATCH→update / DELETE→delete), entity_type, entity_id, new_values (jsonb), ip_address (from X-Forwarded-For / X-Real-IP / CF-Connecting-IP / X-Client-IP), user_agent, request_id (X-Request-Id). Audit write is non-blocking + try/catch-wrapped so failures don't break the user-facing response. Skips GET/HEAD/OPTIONS and any non-2xx response.
- Created `src/lib/auth/mfa.ts` — TOTP utilities using `otplib.authenticator`: `generateMfaSecret(userId, email)` returns `{ secret, qrCodeUrl, qrDataUrl }` (the data URL is a base64 PNG ready for an `<img src>`); `verifyMfaToken(secret, token)` returns boolean (never throws); `generateQrCodeUrl(secret, email)` builds an `otpauth://` URL; `generateBackupCode()` 8-char hex. Also exports a transient pending-secret cache (`setPendingSecret`, `consumePendingSecret`, `peekPendingSecret`) keyed by user_id with 5-min TTL — used by /setup → /verify flow so the secret isn't persisted until the user proves they scanned the QR.
- Created `src/app/api/v1/auth/mfa/setup/route.ts` — POST. Requires authenticated session. Returns 409 if MFA already enabled (call /disable first). Otherwise generates a fresh secret + QR, caches it 5 min server-side keyed by user id, returns `{ secret, qrCodeUrl, qrDataUrl }`. On second /setup call within 5 min, returns the cached secret (so reloads don't force re-scan).
- Created `src/app/api/v1/auth/mfa/verify/route.ts` — POST `{ token: "123456" }`. Two flows share this endpoint: (A) Enable MFA — reads pending secret from cache, verifies TOTP, persists `mfa_secret` + sets `mfa_enabled=true`. (B) Complete MFA-gated login — reads `mfa_secret` from user row, verifies TOTP, returns `{ success, mfa_pending: false, role, permissions }` so the client can re-call signIn() to upgrade the JWT (NextAuth v4 has no server-side JWT rewrite API). Validates token shape with `/^\d{6}$/` regex.
- Created `src/app/api/v1/auth/mfa/disable/route.ts` — POST `{ token: "123456" }`. Requires authenticated session. Defends against session hijack by requiring a CURRENT valid TOTP (the attacker lacks the authenticator device). On valid: clears `mfa_secret` (null) + sets `mfa_enabled=false`. Returns 409 if MFA isn't currently enabled.
- Created `src/app/api/v1/auth/verify-roles/route.ts` — GET dev-only endpoint that verifies B1.4 seed data. Queries `db.role.findMany({ _count: { select: { role_permissions: ... }}})` + `db.permission.count()` + `db.rolePermission.count()`, returns per-role table of `{ code, perm_count, expected, ok }` sorted in the canonical role order. Expected counts are pulled from `ROLE_PERMISSIONS` in role-permissions.ts (the single source of truth): super-admin=27, authority=47, administrator=58, accountant=32, teacher=13, storekeeper=12, guardian=11, student=7. Returns `ok: boolean` overall flag.
- Created `src/app/login/page.tsx` — Client component login page. Two-step flow: (1) credentials form with email + password (with show/hide password toggle), (2) MFA step using `InputOTP` from shadcn/ui (6-digit TOTP). Uses NextAuth's `signIn("credentials", { redirect: false })` client function. On credentials success, fetches `/api/v1/auth/session` to check if mfa_pending — if yes, swaps to the MFA step; if no, redirects to `/dashboard`. On MFA verify success, re-calls `signIn()` to upgrade the JWT to a full session. Design uses ONLY FROZEN tokens: deep-teal primary brand panel (left, hidden on mobile), form panel (right) with shadow-elevation-3, sticky footer at viewport bottom per UI/UX spec. Dev-only hint card at the bottom mentions the seed demo user (administrator@madrasha.local / password123).
- Lint verification: `bun run lint` returns 0 errors, 55 warnings (all pre-existing in `src/components/ui/*` shadcn files — NONE in B2 code). Ran `rg "src/(lib/auth|app/api|app/login|types|middleware)"` against lint output → `NO_ISSUES_IN_B2_CODE`.
- Live verification with curl against the running dev server on port 3000:
  - `GET /login` → 200 (33 KB HTML)
  - `GET /api/auth/providers` → 200 JSON (NextAuth provider list — confirms NextAuth mounted)
  - `GET /api/v1/auth/session` (no cookie) → 401 JSON `{"error":"Unauthorized","message":"Authentication required. Sign in at /login."}` (middleware gate ✓)
  - `GET /api/v1/auth/verify-roles` (no cookie) → 401 JSON (same gate ✓)
  - `POST /api/v1/auth/mfa/setup` (no cookie) → 401 JSON (gate ✓)
  - `POST /api/v1/auth/mfa/verify` (no cookie) → 401 JSON (gate ✓)
  - `POST /api/v1/auth/mfa/disable` (no cookie) → 401 JSON (gate ✓)
  - `POST /api/v1/donations` (no cookie) → 404 (NOT 401 — middleware correctly bypassed per Risk R10; the route handler itself doesn't exist yet because donations are a separate task) ✓

Stage Summary:
- **B2.1 NextAuth Setup**: ✓ config.ts + route handler + password.ts + tokens.ts + middleware.ts + /api/v1/auth/session/route.ts. JWT strategy with 15-min access TTL, role/tenant/permissions injected into JWT, bcrypt password verification, account lockout (5 fails → 15-min lock), MFA-pending gate integrated.
- **B2.2 Permission Middleware**: ✓ with-permission.ts (withPermission + withPermissions AND + withAnyPermission OR), with-tenant.ts (getTenantContext + hasPermission + tenantWhere helper), with-idempotency.ts (in-memory Map cache, 24h TTL, X-Idempotent-Replay header), with-audit.ts (writes to audit_logs table with full actor + entity + IP + user-agent + request-id metadata).
- **B2.3 MFA TOTP**: ✓ mfa.ts (otplib + qrcode), /api/v1/auth/mfa/setup (generates + caches secret 5 min, returns QR data URL), /api/v1/auth/mfa/verify (two flows: enable + login-complete; persists secret on success), /api/v1/auth/mfa/disable (requires current TOTP to disable, defends against session hijack). NextAuth config integrated: mfa_pending=true → empty permissions[] → withPermission middleware rejects every API call.
- **B2.4 Verify Roles**: ✓ /api/v1/auth/verify-roles returns per-role permission count table with `ok` flag per role + `ok: boolean` overall, expected counts sourced from role-permissions.ts (single source of truth). Login page uses shadcn/ui Card + Input + Button + Alert + InputOTP components, FROZEN tokens only (deep teal + warm gold accent), sticky footer pattern.
- **Lint**: 0 errors. 55 warnings all pre-existing in `src/components/ui/*` shadcn files (raw px values in className — to be addressed in a future design-QA pass, not a B2 concern).
- **Files created (14 total)**:
  1. src/lib/auth/password.ts
  2. src/lib/auth/tokens.ts
  3. src/lib/auth/config.ts
  4. src/lib/auth/with-permission.ts
  5. src/lib/auth/with-tenant.ts
  6. src/lib/auth/with-idempotency.ts
  7. src/lib/auth/with-audit.ts
  8. src/lib/auth/mfa.ts
  9. src/types/next-auth.d.ts
  10. src/app/api/auth/[...nextauth]/route.ts
  11. src/app/api/v1/auth/session/route.ts
  12. src/app/api/v1/auth/mfa/setup/route.ts
  13. src/app/api/v1/auth/mfa/verify/route.ts
  14. src/app/api/v1/auth/mfa/disable/route.ts
  15. src/app/api/v1/auth/verify-roles/route.ts
...
Task ID: B4.1
Agent: full-stack-developer
Task: Build B4.1 — Student API (CRUD + promotion + history). Four routes implementing list/create/read/update/soft-delete + dedicated promote endpoint (Risk R4) + history timeline. Zod schemas appended to validation/schemas.ts.

Work Log:
- Read worklog.md (15/36 sessions done, B0-B3 complete) and studied 5 pattern files: branches/route.ts (GET/POST), branches/[id]/route.ts (GET/PATCH/DELETE with async params), with-permission.ts (withPermission HOF + 401/403 contract), with-tenant.ts (getTenantContext + tenantWhere helper), api/helpers.ts (jsonResponse/errorResponse/successResponse/paginatedResponse/parsePagination), validation/schemas.ts (Zod append pattern). Cross-referenced Prisma schema for `model Student` (706-762) + `model StudentHistory` (1258-1294) + `enum Gender` (male|female) + `enum StudentStatus` (active|graduated|withdrawn) + `model StudentGuardian` + `model FeePlan` + `model FeeInstallment` to ensure all writes match DB columns exactly. Confirmed permission codes students.view/create/update/promote exist in prisma/seed.ts.
- Appended 5 schemas to src/lib/validation/schemas.ts (inserted between toggleModuleSchema and the Employee B4.4 block — no existing files modified): studentGenderSchema (enum male|female), studentStatusSchema (enum active|graduated|withdrawn), createStudentSchema (code optional, name/name_bn required, class_id/guardian_id UUIDs, roll int, gender enum, dob coerce.date, optional blood_group/present_address/permanent_address + guardian_relation defaulting to "father"), updateStudentSchema (omit code → partial → extend with status/special_notes/photo_url/previous_school/blood_donor/phone/email), promoteStudentSchema ({ to_class_id uuid, to_section_id uuid?, effective_date coerce.date, reason 1..1000 }).
- Created src/app/api/v1/students/route.ts — GET wrapped in withPermission("students.view") returns paginatedResponse with class/section/primary_guardian objects, filters by search (name|name_bn|code case-insensitive), class_id, section (interpreted as section_id per task spec), status, guardian_id (via student_guardians.some). POST wrapped in withPermission("students.create") validates body, cross-checks class+guardian+section exist in org, checks roll+code uniqueness for clean 409s instead of raw P2002, auto-generates `MOS-{year}-{seq}` code when omitted (count-based, zero-padded to 4 digits), atomically creates Student + StudentGuardian (is_primary=true, relation from body or "father") + seeds an initial StudentHistory row with action="admitted" (bootstraps the Risk R4 timeline panel).
- Created src/app/api/v1/students/[id]/route.ts — GET returns full profile (student fields + class + section + all guardians with relation/primary flag + fee_plans array where each plan carries an aggregated summary {billed, paid, balance, installments_total, installments_paid}). PATCH validates updateStudentSchema, re-checks roll uniqueness in destination section if roll/section_id changes, verifies class/section still belong to org on cross-class moves. DELETE soft-deletes (status="withdrawn" + deleted_at=now) and appends a `withdrawn` StudentHistory entry — StudentHistory rows are NEVER touched by DELETE per Risk R4 (timeline invariants preserved).
- Created src/app/api/v1/students/[id]/promote/route.ts — POST wrapped in withPermission("students.promote"). Fetches current student (to capture from_class_id/from_section_id for the history row), verifies destination class+section belong to org, checks roll-collision in destination section if it differs from current section. Wraps the two writes in `db.$transaction([historyInsert, studentUpdate])` so the timeline stays consistent with the live Student row — partial failure rolls back. History row is action="promoted" with remark=reason, effective_date=effective_date, academic_year derived from effective_date.getFullYear(). Old assignment preserved in history (Risk R4 — never deleted, rendered as past chip).
- Created src/app/api/v1/students/[id]/history/route.ts — GET wrapped in withPermission("students.view"). First verifies student exists in current tenant (prevents cross-tenant probing — returns 404 for unknown IDs). Returns timeline array ordered by effective_date DESC then created_at DESC. Each record: { id, action, from_class, from_section, to_class, to_section, effective_date, reason (mapped from remark), academic_year, action_by: {id, name}, created_at }. Response shape { student: {...}, timeline: [...], total }. Intentionally does NOT filter deleted_at — Risk R4 invariant says history rows are never deleted; the include-all approach lets a future admin soft-delete an erroneous entry and have the frontend render it as a "redacted" chip.
- Ran `bun run lint` — 0 errors, 55 warnings (all pre-existing raw-tokens lints in components/ui, components/dev, components/shell, components/finance — NONE in my new files under src/app/api/v1/students/).
- Smoke-tested all 7 endpoints via curl without auth — every one returned 401 with JSON body `{"error":"Unauthorized","message":"Authentication required. Sign in at /login."}`:
    GET  /api/v1/students                    → 401
    GET  /api/v1/students/abc               → 401
    GET  /api/v1/students/abc/history       → 401
    POST /api/v1/students                   → 401
    POST /api/v1/students/abc/promote       → 401
    PATCH /api/v1/students/abc             → 401
    DELETE /api/v1/students/abc             → 401

Stage Summary:
- Files created (4): src/app/api/v1/students/route.ts, src/app/api/v1/students/[id]/route.ts, src/app/api/v1/students/[id]/promote/route.ts, src/app/api/v1/students/[id]/history/route.ts.
- Files modified (1, append-only): src/lib/validation/schemas.ts — appended studentGenderSchema, studentStatusSchema, createStudentSchema, updateStudentSchema, promoteStudentSchema.
- All routes use ONLY existing patterns: withPermission / getTenantContext / tenantWhere / jsonResponse / errorResponse / successResponse / paginatedResponse / parsePagination / Zod safeParse.
- Risk R4 lock-in implemented end-to-end:
  1) Promotion wizard history timeline is fed by GET /api/v1/students/:id/history — ordered DESC by effective_date.
  2) Old assignment is preserved as a StudentHistory row (action="promoted", from_class_id/from_section_id = previous, to_class_id/to_section_id = new). DELETE flow also appends a `withdrawn` row. POST /students seeds an `admitted` row on creation.
  3) StudentHistory rows are NEVER deleted by any flow in this task. Soft-deleted history rows would still appear in the timeline (intentional — supports future "redacted" chip rendering).
  4) The promote flow uses db.$transaction so the history INSERT and the Student UPDATE are atomic — partial failure cannot leave the timeline inconsistent with the live assignment.
- HTTP status codes (smoke-tested, unauthenticated): GET=401, POST=401, PATCH=401, DELETE=401, POST promote=401, GET history=401 — all match the `withPermission` HOF contract (session missing → 401 JSON).
- Lint: 0 errors / 55 pre-existing warnings in unrelated files. No new warnings introduced.
- What CANNOT be tested without a running PostgreSQL: the full happy-path POST /students (db.student.create would throw on unreachable DB → 500 surfaced, but the withPermission gate already short-circuits to 401 for unauthenticated requests, so the lint+smoke-test signal is sufficient per project convention). To smoke-test the full flow: `docker compose up -d && bunx prisma db push && bunx prisma db seed`, then sign in as administrator@madrasha.local and POST a student.

---
Task ID: B4.4
Agent: Z.ai Code (full-stack-developer)
Task: Build B4.4 — Employee API. GET (list) + POST (create) at /api/v1/employees; GET (single) + PATCH (update) + DELETE (soft + disable linked User) at /api/v1/employees/:id. Appended `createEmployeeSchema` + `updateEmployeeSchema` to validation/schemas.ts. Implements SRS §2.2.6 "resigned employee cannot log in" via cascading User.status="disabled" on soft delete.

Work Log:
- Read worklog.md (B0-B3 complete, 15/36 sessions done) + key pattern files: branches/route.ts (GET list + withPermission POST pattern), branches/[id]/route.ts (RouteContext = { params: Promise<{ id: string }> } + await ctx.params + withPermission PATCH/DELETE pattern), with-permission.ts (withPermission HOF + RequestHandler type — handler signature variance is pre-existing), with-tenant.ts (getTenantContext returns TenantContext { organization_id, branch_id, user_id, role, permissions } — super-admin/authority-without-branch_id get org-wide scope), api/helpers.ts (jsonResponse, errorResponse, successResponse, paginatedResponse, parsePagination), validation/schemas.ts (Zod pattern — appended after the Module Toggle section, BEFORE the existing B4 People section so the file stays section-ordered), prisma/schema.prisma (Employee model: organization_id, branch_id?, user_id, employee_code, designation, department?, joined_at @db.Date, left_at?, salary Decimal(14,2), status default "active", nid_number?, photo_url?, phone? — has @@unique([organization_id, employee_code]) + @@unique([user_id]) so each Employee ↔ User is 1:1 and codes are unique per org).
- Verified Employee is the people-layer record for non-teaching staff: designation field holds granular job title (accountant | librarian | storekeeper | clerk | driver | cook | guard | …), User row holds the login identity. The 8 seeded RoleCodes (super_admin/authority/administrator/accountant/teacher/storekeeper/guardian/student) do NOT include an "employee" code; the seed maps employees to the "storekeeper" role. Per docs/DATA_DICTIONARY.md line 176: "employees (optional — when role is administrator/accountant/storekeeper)".
- Appended to src/lib/validation/schemas.ts (append-only — pre-existing schemas untouched):
  * `employeeStatusSchema` — `z.enum(["active", "on-leave", "resigned"])` matching the SRS §2.2.6 lifecycle.
  * `createEmployeeSchema` — name, name_bn?, designation, phone, email, salary?, joining_date (ISO 8601 datetime), branch_id? (uuid), role_code? (override default "storekeeper"), temp_password? (admin-supplied; otherwise server-generated).
  * `updateEmployeeSchema` — partial of createEmployeeSchema + status + extra HR fields (department, nid_number, photo_url) + nullable branch_id (allow unassign).
- Created src/app/api/v1/employees/route.ts:
  * GET (perm: employees.view) — paginated list, query params page/pageSize/search/status. Tenant scoping: super-admin sees all orgs; authority-without-branch_id sees all branches in their org; branch roles see only their own branch. Search is case-insensitive substring on designation OR user.name OR user.email. Joins with User + Branch tables so the UI can render email + status badge + branch name without a second round-trip. Returns paginatedResponse envelope.
  * POST (perm: employees.create) — body validated by createEmployeeSchema. Pre-flight checks: (1) branch_id (if provided) must exist in the current org → 404 otherwise; (2) email must not collide with an existing user in the org → 409 otherwise (clean error instead of raw Prisma P2002); (3) role_code (defaults to "storekeeper") must resolve to a Role row in the current org → 422 otherwise. Auto-generates employee_code as `EMP-{year}-{seq padded 4}` by counting existing rows with the same year prefix. Generates a 12-char temp password (generateTempPassword from src/lib/auth/password.ts — uses crypto.randomInt per SRS §6.2.3 R-S2) and hashes it with hashPassword (bcrypt 10 rounds). Creates User + Employee rows in a db.$transaction so a failure in either step rolls both back. Returns successResponse with the new Employee + a one-time `temp_password` field (NEVER persisted in plaintext; the admin must hand it to the new employee out-of-band per SRS §6.2.3 R-S3). Catches Prisma P2002 → 409 "Employee code or email collision — please retry" (covers race conditions on employee_code generation).
- Created src/app/api/v1/employees/[id]/route.ts:
  * GET (perm: employees.view) — single employee with linked User (incl. role code/name, mfa_enabled, last_login_at, last_login_ip) + Branch info. Tenant-scoped (404 if the employee belongs to a different org/branch).
  * PATCH (perm: employees.create) — body validated by updateEmployeeSchema. Splits the patch into User-side fields (name, name_bn, email, phone) and Employee-side fields (designation, salary, joined_at, branch_id, department, nid_number, photo_url, status). When status transitions to "resigned", auto-sets left_at. Email uniqueness re-checked when email changes (409 if another user in the org has it). Branch_id (if changed) re-validated against the current org (404 otherwise). Runs both updates in a db.$transaction so partial failure is impossible. Catches P2002 → 409 "Email already in use by another user".
  * DELETE (perm: employees.create) — soft-deletes the Employee AND disables the linked User account so the resigned staff member cannot log in (per SRS §2.2.6). Employee: { status: "resigned", left_at: now, deleted_at: now, updated_by }. User: { status: "disabled", updated_by }. Both writes run in a db.$transaction. Idempotent: if already soft-deleted, returns 200 "Employee already deleted (soft)" without re-writing.
- Inline `employeeWhere(ctx, id)` helper at the top of [id]/route.ts builds the tenant-scoped Prisma `where` clause — branch roles get a branch_id constraint, authority + super-admin don't. Same shape as branches/[id]/route.ts but branch-aware (employees have branch_id? whereas branches don't).
- Lint verification: `cd /home/z/my-project && bun run lint` → 0 errors, 55 warnings. ALL 55 warnings are pre-existing in `src/components/ui/*` shadcn files + `src/components/dev/*` + `src/components/finance/CollectPaymentDialog.tsx` + `src/components/shell/TopBar.tsx` (raw px/hex values per madrasha/no-raw-tokens rule — known design-QA debt from earlier phases, NOT a B4.4 concern). Confirmed via `bun run lint 2>&1 | rg "api/v1/employees|validation/schemas"` → `NO_ISSUES_IN_B4_4_CODE`. Zero new warnings introduced.
- TypeScript verification (`npx tsc --noEmit`): 4 errors total, ALL match pre-existing patterns:
  * `employees/[id]/route.ts(160,3)` + `(386,3)` — RequestHandler variance: `Promise<{ id: string }>` not assignable to `Promise<Record<string, string>>` because Promises are invariant. EXACT same pattern as `branches/[id]/route.ts(76,3)` + `(126,3)` — verified by running tsc on branches which produces the same error. Since the task forbids modifying existing files (with-permission.ts), I matched the established convention. Runtime is unaffected — the handler still receives `{ params: Promise<{ id: string }> }` and correctly `await`s it.
  * `validation/schemas.ts(23,15)` — pre-existing in `updateOrganizationSchema.settings: z.record(z.unknown())` (newer Zod requires `z.record(keyType, valueType)`). NOT in my appended schemas — line 23 is in code I did not touch.
  * `validation/schemas.ts(159,19)` — pre-existing in B4.1 `registerStudentSchema.dob: z.string().iso().optional()` (line 159 is in the B4.1 student schema section, NOT in my appended Employee schemas).
- Curl verification against the running dev server on port 3000 (no session cookie → middleware returns 401 JSON):
  * `GET /api/v1/employees` → 401 `{"error":"Unauthorized","message":"Authentication required. Sign in at /login."}` (gate ✓)
  * `GET /api/v1/employees?page=1&pageSize=10&search=ali&status=active` → 401 (gate ✓ — query params don't bypass auth)
  * `POST /api/v1/employees` (empty body) → 401 (gate ✓ — fires before Zod validation)
  * `GET /api/v1/employees/00000000-0000-0000-0000-000000000000` → 401 (gate ✓)
  * `PATCH /api/v1/employees/00000000-0000-0000-0000-000000000000` (empty body) → 401 (gate ✓)
  * `DELETE /api/v1/employees/00000000-0000-0000-0000-000000000000` → 401 (gate ✓)
  * All 5 endpoints mounted at canonical paths + middleware intercepts unauthenticated requests correctly. NO 500/compile errors in dev.log (verified via `tail -30 dev.log | grep -iE "error|warn|fail|exception"` → `NO_ERRORS_IN_DEV_LOG`).
- End-to-end DB tests couldn't be run because PostgreSQL isn't running in this sandbox. To smoke-test: `docker compose up -d && bunx prisma db push && bunx prisma db seed`, log in at /login as `administrator@madrasha.local` / `password123`, then hit the 5 endpoints with the session cookie. POST first to create a few employees (the seed doesn't seed Employee rows directly — only Users with the storekeeper role), then GET the list to verify pagination + search filters.

Stage Summary:
- **Files created (2 new — zero existing files modified except schemas.ts append)**:
  1. `src/app/api/v1/employees/route.ts` — GET list (paginated, tenant-scoped, search + status filters) + POST create (provisions User + Employee in a transaction, auto-generates EMP-{year}-{seq} code, returns one-time temp_password).
  2. `src/app/api/v1/employees/[id]/route.ts` — GET single (with linked User + Branch + Role) + PATCH update (split User/Employee patch, runs in transaction) + DELETE soft-delete (cascades to User.status="disabled" so resigned staff cannot log in per SRS §2.2.6).
- **Files modified (1, append-only)**:
  3. `src/lib/validation/schemas.ts` — appended `employeeStatusSchema`, `createEmployeeSchema`, `updateEmployeeSchema` after the Module Toggle section (B3.2) and before the existing B4 People section. Pre-existing schemas untouched.
- **Endpoints (5 total)**: GET/POST /employees, GET/PATCH/DELETE /employees/:id.
- **Permissions enforced**: `employees.view` (GET list, GET single), `employees.create` (POST, PATCH, DELETE — same as branches which uses `organization.branch.create` for both create + update + delete).
- **HTTP status codes**: 200 (GET list paginated, GET single, PATCH, DELETE); 201 implicit via successResponse wrapper; 400 (validation failed, invalid JSON body, invalid joining_date); 401 (no session — middleware gate fires first); 403 (session exists but lacks required permission); 404 (employee/branch not found in current tenant scope); 409 (email collision, employee_code P2002 race); 422 (role_code unresolvable in current org).
- **SRS §2.2.6 enforced**: DELETE soft-deletes the Employee (status="resigned" + left_at + deleted_at) AND disables the linked User account (status="disabled") so the resigned staff member can no longer log in. Both writes run in a db.$transaction so partial failure is impossible. The auth CredentialsProvider already rejects users with status !== "active" per `src/lib/auth/config.ts`.
- **Lint**: 0 errors. 55 warnings all pre-existing in shadcn/ui components + dev pages (raw px values in className — design-QA debt, not B4.4 concern). Zero new warnings introduced.
- **Curl**: all 5 endpoints return 401 JSON for unauthenticated requests (matches middleware gate + branches pattern). Authenticated smoke-test requires Postgres to be running (currently down in sandbox).
- **TS strictness**: 4 tsc errors in new files — ALL match pre-existing patterns: 2 RequestHandler variance issues (same as branches/[id]/route.ts), 1 pre-existing `z.record(z.unknown())` in updateOrganizationSchema, 1 pre-existing `z.string().iso()` in B4.1 registerStudentSchema. No new TS patterns introduced.
- **Notes for the next session (B5 / Academic API or B4.5 if any)**:
  * The Employee API is now ready for the upcoming Employee Management UI to wire its list + create + edit + resign flows.
  * The POST response includes a one-time `temp_password` field — the frontend MUST display it once and warn the admin it won't be shown again. Use a `<PasswordReveal>` component with a "Copy" button.
  * The DELETE response includes `{ status: "resigned", deleted_at, user_disabled: true }` — the frontend can show a confirmation toast like "Employee resigned; login access revoked" without re-fetching.
  * The PATCH endpoint accepts `branch_id: null` to unassign from a branch (e.g. when an employee transfers to a org-level role).
  * The default `role_code` for new employees is "storekeeper" (the closest existing non-teaching staff role). The frontend could surface a role picker populated from GET /api/v1/roles (filtered to non-teaching system roles) to let the admin override — e.g. assign "accountant" role to a finance clerk.

---
Task ID: B4.3
Agent: full-stack-developer
Task: Build B4.3 — Guardian + Teacher API. Created 7 route files (12 endpoints) + appended 5 Zod schemas. Implements the full people-management CRUD surface for Guardians (3 endpoints + nested children) + Teachers (4 endpoints incl. duplicate-active-assignment block on /teachers/assign with structured 409). Guardian-role users see only their own row via `guardians.view.own` scope filtering (defense-in-depth 404 to prevent ID enumeration).

Work Log:
- Read worklog.md (B0-B3 complete, B3.4 was the latest) + key reference files for patterns: branches/route.ts (GET/POST + tenantWhere + Zod safeParse + 409 on uniqueness collision), branches/[id]/route.ts (async params `Promise<{ id: string }>` + withPermission PATCH/DELETE + soft-delete), with-permission.ts (withPermission / withAnyPermission / withPermissions HOFs + RouteContext type), with-tenant.ts (getTenantContext + tenantWhere super-admin / authority / branch-level scoping), api/helpers.ts (jsonResponse / errorResponse / successResponse / paginatedResponse / parsePagination), validation/schemas.ts (Zod schema append-only convention), prisma/schema.prisma — inspected Guardian (fields + @@unique([organization_id, phone]) + user_id @unique), Teacher (fields + @@unique([organization_id, employee_code]) + @@unique([user_id])), TeacherAssignment (fields + @@unique([organization_id, academic_year, teacher_id, class_id, section_id, subject_id])), StudentGuardian junction (relation / is_primary / can_pickup / receive_sms / receive_email), Section (teacher_id is the canonical class-teacher designation), Class + Subject (for the assign endpoint pre-flight checks). Verified the role-permissions.ts map: Guardian role has only `guardians.view.own` (NOT `guardians.view`); Teacher role has `guardians.view`; administrator / authority / accountant roles have `guardians.view`; administrator / authority roles have `teachers.view` + `teachers.create` + `teachers.assign`.
- Appended 5 Zod schemas to `src/lib/validation/schemas.ts` (People section, after the existing Module Toggle section — did NOT touch any pre-existing schema):
  * `createGuardianSchema` — `{ name, name_bn?, phone, email?, occupation?, relation (enum 6 values, default father), nid_number?, annual_income?, is_primary?, address?, user_id?, branch_id? }`.
  * `updateGuardianSchema` — `createGuardianSchema.partial().extend({ relation: enum.optional() })` (relation re-declared explicitly to drop the `.default("father")` from the create schema — PATCH should not silently mutate relation if the field is absent).
  * `createTeacherSchema` — `{ user_id (uuid, required), employee_code (regex alphanumeric+dash), designation?, qualification?, specialization?, joined_at (datetime ISO), salary?, status (enum 3 values, default active), phone?, nid_number?, photo_url?, branch_id? }`.
  * `updateTeacherSchema` — `createTeacherSchema.omit({ user_id: true }).partial().extend({ status: enum.optional(), left_at: datetime.nullable.optional() })` (user_id is immutable after create — Teacher ↔ User is 1:1).
  * `assignTeacherSchema` — `{ teacher_id, class_id, section_id?.nullable, subject_id, is_class_teacher?, academic_year? (int 2000-2100, defaults to current year server-side), notes? }`.
- Created `src/app/api/v1/guardians/route.ts`:
  * GET (perm: guardians.view OR guardians.view.own — via `withAnyPermission`) — paginated list with `parsePagination(url)`. `search` query matches name / name_bn / phone / email case-insensitively (Prisma `mode: "insensitive"`). Tenant scoping via `tenantWhere(ctx)` + soft-delete filter. **Guardian-scope rule**: if caller has `guardians.view.own` AND NOT `guardians.view`, the where-clause adds `user_id = ctx.user_id` so Guardian-role users see ONLY their own guardian row. Each row carries `children_count` via `_count.student_guardians` (filtered to `deleted_at: null`). Returns `paginatedResponse`.
  * POST (perm: students.create — admin creates guardians when adding students) — body validated by createGuardianSchema. Pre-flight 409 checks: (1) `[organization_id, phone]` collision (matches the @@unique constraint → clean 409 instead of raw Prisma P2002), (2) if `user_id` supplied → verify User exists in current tenant (404) AND isn't already linked to another Guardian (409), (3) if `branch_id` supplied → verify Branch in current org (404). `branch_id` defaults to `ctx.branch_id` if not supplied (so admins at a branch create branch-scoped guardians without explicitly passing branch_id). Returns `successResponse(guardian, "Guardian created")`.
- Created `src/app/api/v1/guardians/[id]/route.ts`:
  * GET (perm: guardians.view OR guardians.view.own) — single guardian + nested `children` array (joins via `student_guardians` → `student` with class_id/section_id/roll/status/photo_url). **Defense-in-depth scope check**: for `guardians.view.own`-only users, returns 404 (NOT 403) if `guardian.user_id !== ctx.user_id` — so an attacker can't enumerate guardian IDs to discover who has guardian portal access. The 404 matches the "not in scope" semantics cleanly.
  * PATCH (perm: students.create) — body validated by updateGuardianSchema. Re-validates `[organization_id, phone]` uniqueness if `phone` is being changed (409 on collision, scoped via `NOT: { id }`). Re-validates user_id 1:1 invariant if `user_id` is being changed (409 on collision). Writes `updated_by = ctx.user_id`.
  * DELETE (perm: students.create) — soft-delete in a `db.$transaction`: (1) soft-delete all `StudentGuardian` rows where `guardian_id = id` AND `deleted_at = null` (so no orphan active links remain — keeps the junction table consistent for future student→guardian queries), (2) soft-delete the Guardian row itself (`deleted_at = now()`, `is_primary = false`). Returns `successResponse(null, "Guardian deleted (soft)")`.
- Created `src/app/api/v1/guardians/[id]/children/route.ts`:
  * GET (perm: guardians.view OR guardians.view.own) — returns `{ guardian: { id, name }, children: [{ link_id, relation, is_primary, can_pickup, receive_sms, receive_email, student: { id, code, name, name_bn, class_id, section_id, roll, gender, status, photo_url, class: { id, name, name_bn }, section: { id, name } } }], total }`. Query joins through `student_guardians.student` → `class` + `section` so the frontend can render the child table without a second round-trip. Same `guardians.view.own` defense-in-depth 404 scope check as the single-guardian GET.
- Created `src/app/api/v1/teachers/route.ts`:
  * GET (perm: teachers.view) — paginated list with optional `search` (matches `employee_code` / `specialization` / `designation` / `phone`, AND joined `user.name` / `name_bn` / `email` / `phone`) + optional `status` filter. Each row carries `active_assignments` count via `_count.teacher_assignments` (filtered to `{ is_active: true, deleted_at: null }`). Response field `name` / `name_bn` come from the linked User row (Teacher has no own name field — it inherits from User). `photo_url` falls back to `user.avatar_url` if Teacher.photo_url is null.
  * POST (perm: teachers.create) — body validated by createTeacherSchema. The body MUST supply `user_id` (Teacher.user_id is NOT optional in the Prisma schema — 1:1 with User). Pre-flight 409 checks: (1) User exists in current tenant (404 otherwise), (2) `[organization_id, employee_code]` collision (matches @@unique → 409), (3) `user_id` 1:1 invariant (matches @@unique([user_id]) → 409 with `{ teacher_id, employee_code }` so the frontend can render a "User is already linked to Teacher T-002" toast), (4) if `branch_id` supplied → verify Branch in current org (404). `joined_at` accepted as ISO 8601 string → converted to Date in the route handler (Prisma wants DateTime).
- Created `src/app/api/v1/teachers/[id]/route.ts`:
  * GET (perm: teachers.view) — single teacher + nested `assignments` array (only active + non-deleted assignments, joined with class + section + subject names). Response also includes `assignments_count` for convenience.
  * PATCH (perm: teachers.create) — body validated by updateTeacherSchema (which `.omit({ user_id: true })` — user_id is immutable after create). Re-validates `[organization_id, employee_code]` uniqueness if `employee_code` is being changed (409 on collision, scoped via `NOT: { id }`). Special handling: if `status` is being changed to `"resigned"` and no `left_at` was supplied → auto-set `left_at = new Date()` (matches the SRS §8.x employee lifecycle convention). `joined_at` and `left_at` accepted as ISO strings and coerced to Date objects for Prisma (schema permits `left_at: nullable`).
- Created `src/app/api/v1/teachers/assign/route.ts`:
  * POST (perm: teachers.assign) — body validated by assignTeacherSchema: `{ teacher_id, class_id, section_id?, subject_id, is_class_teacher?, academic_year?, notes? }`.
  * **Risk R-DUP** (matches the frontend duplicate-active-assignment inline rule): explicit `findFirst` against `{ teacher_id, class_id, section_id, subject_id, academic_year, is_active: true, deleted_at: null }`. If a match exists → return **409** with structured body so the frontend can render the inline AlertCard without re-fetching:
    ```json
    { "error": "Teacher is already assigned to this class+section+subject for the academic year",
      "details": { "existing_assignment_id": "...", "existing_created_at": "...", "teacher_id": "...", "class_id": "...", "section_id": null, "subject_id": "...", "academic_year": 2026 } }
    ```
  * Validates `is_class_teacher === true && section_id === null` → **422** with hint ("Provide section_id when is_class_teacher is true, or omit is_class_teacher"). The Section model designates its class teacher via `Section.teacher_id` (one teacher per section, NOT on the TeacherAssignment row) — so a section_id is structurally required to set a class teacher.
  * Parallel pre-flight existence checks (4 parallel `findFirst` calls): teacher, class, section (if supplied — also verified to belong to the given class_id), subject. Returns 404 for any missing/unscoped entity.
  * `academic_year` defaults to `new Date().getFullYear()` if not supplied — so callers don't have to thread it through every form submission.
  * Wrapped in `db.$transaction`: (1) create the `TeacherAssignment` row (`is_active: true`), (2) if `is_class_teacher === true` AND the section's current `teacher_id !== this teacher.id` → update `Section.teacher_id` (idempotent — skips the write if the section already has this teacher as its class teacher). Response: `successResponse({ ...assignment, teacher: { id, employee_code, name }, class, section, subject, is_class_teacher }, "Teacher assigned")`.
- Created `src/app/api/v1/teachers/[id]/assignments/route.ts`:
  * GET (perm: teachers.view) — returns `{ teacher: { id, employee_code, name, name_bn, designation, specialization }, assignments: [{ id, academic_year, is_active, notes, created_at, updated_at, class: {...}, section: {...}, subject: {...}, is_class_teacher }], total, active_count }`. Query params: `status` ∈ {active, inactive, all} (default active), `year` (integer academic year filter). `is_class_teacher` is computed per-assignment: `section.teacher_id === teacher.id` — matches the Section model's class-teacher designation. This means the frontend doesn't need a separate "is this teacher the class teacher of any section" query.
- Lint verification: `cd /home/z/my-project && bun run lint` → **0 errors, 55 warnings** (ALL pre-existing in `src/components/ui/*` shadcn files + `src/components/dev/*` + `src/components/{shell,finance,dev}/*` — raw px/hex values per madrasha/no-raw-tokens, known design-QA debt from earlier phases, NOT a B4.3 concern). Filtered lint output for my new paths: `bun run lint 2>&1 | grep -E "src/app/api/v1/guardians|src/app/api/v1/teachers|src/lib/validation/schemas"` → `NO_LINT_ISSUES_IN_B4_3_CODE`. Zero new warnings introduced.
- Live verification with curl against a temp dev server on port 3000 (spawned via `setsid bun run dev &` so the bun process survives the bash session — same trick the B3.3 / B3.4 sessions used). All 12 endpoints return 401 JSON `{"error":"Unauthorized","message":"Authentication required. Sign in at /login."}` — the middleware gates all `/api/v1/*` requests before the route handler runs:
  • GET /api/v1/guardians                                                                → 401 ✓
  • POST /api/v1/guardians                                                                → 401 ✓
  • GET /api/v1/guardians/00000000-0000-0000-0000-000000000000                           → 401 ✓
  • PATCH /api/v1/guardians/00000000-0000-0000-0000-000000000000                          → 401 ✓
  • DELETE /api/v1/guardians/00000000-0000-0000-0000-000000000000                         → 401 ✓
  • GET /api/v1/guardians/00000000-0000-0000-0000-000000000000/children                   → 401 ✓
  • GET /api/v1/teachers                                                                 → 401 ✓
  • POST /api/v1/teachers                                                                → 401 ✓
  • GET /api/v1/teachers/00000000-0000-0000-0000-000000000000                            → 401 ✓
  • PATCH /api/v1/teachers/00000000-0000-0000-0000-000000000000                          → 401 ✓
  • POST /api/v1/teachers/assign                                                         → 401 ✓
  • GET /api/v1/teachers/00000000-0000-0000-0000-000000000000/assignments                 → 401 ✓
  • No 500/compile errors in /tmp/dev-b43.log — Turbopack compiled each route on first hit, all returned 401 cleanly.
  • Stopped the temp dev server after verification — the auto-dev-server managed by the sandbox environment will continue running normally.
- End-to-end DB tests couldn't be run because PostgreSQL isn't running in this sandbox. To smoke-test end-to-end: `docker compose up -d && bunx prisma db push && bunx prisma db seed`, log in at /login as `administrator@madrasha.local` / `password123`, then hit the 12 endpoints with the session cookie — GET /guardians should return paginated guardians with `children_count`, POST /guardians should create a guardian and enforce the [org, phone] uniqueness, POST /teachers/assign should return 409 on a duplicate-active-assignment attempt with structured `details`, GET /teachers/:id should return the teacher + nested assignments, and DELETE /guardians/:id should soft-delete the guardian AND cascade-soft-delete the StudentGuardian links in one transaction.

Stage Summary:
- **Files created (7 new — zero existing files modified except schemas.ts append-only)**:
  1. `src/app/api/v1/guardians/route.ts` (GET list with Guardian-role own-scope + POST create with 409 phone-collision + 409 user_id 1:1 checks)
  2. `src/app/api/v1/guardians/[id]/route.ts` (GET single with nested children + PATCH + DELETE soft-delete with StudentGuardian cascade)
  3. `src/app/api/v1/guardians/[id]/children/route.ts` (GET children with class+section joins + defense-in-depth 404 scope check)
  4. `src/app/api/v1/teachers/route.ts` (GET list with user-join search + POST create with 409 employee_code + 409 user_id 1:1 checks)
  5. `src/app/api/v1/teachers/[id]/route.ts` (GET single with nested assignments + PATCH with resigned→left_at auto-set)
  6. `src/app/api/v1/teachers/assign/route.ts` (POST with Risk R-DUP duplicate-active-assignment 409 + is_class_teacher→Section.teacher_id update in transaction + 422 when is_class_teacher without section_id)
  7. `src/app/api/v1/teachers/[id]/assignments/route.ts` (GET with status + year filters + computed is_class_teacher per row)
- **Files modified (1, append-only)**:
  8. `src/lib/validation/schemas.ts` — appended `createGuardianSchema`, `updateGuardianSchema`, `createTeacherSchema`, `updateTeacherSchema`, `assignTeacherSchema` after the existing Module Toggle section. Pre-existing schemas untouched.
- **Endpoints (12 total)**:
  - Guardian (5): GET/POST /guardians, GET/PATCH/DELETE /guardians/:id, GET /guardians/:id/children
  - Teacher (7): GET/POST /teachers, GET/PATCH /teachers/:id, POST /teachers/assign, GET /teachers/:id/assignments
- **Permissions enforced**: students.create (POST/PATCH/DELETE /guardians), guardians.view OR guardians.view.own (GET /guardians + GET /guardians/:id + GET /guardians/:id/children), teachers.view (GET /teachers + GET /teachers/:id + GET /teachers/:id/assignments), teachers.create (POST /teachers + PATCH /teachers/:id), teachers.assign (POST /teachers/assign).
- **Guardian-role own-scope**: implemented as `where.user_id = ctx.user_id` on the list endpoint + defense-in-depth 404 (not 403) on the single + children endpoints so Guardian-role users can only see / fetch their OWN guardian row (linked via Guardian.user_id). The 404 prevents guardian-ID enumeration.
- **Risk R-DUP**: the duplicate-active-assignment block on POST /teachers/assign returns a structured 409 with `details: { existing_assignment_id, teacher_id, class_id, section_id, subject_id, academic_year }` so the frontend can render the "already assigned" inline AlertCard without re-fetching. This is the same structured-409 pattern B3.2 used for Risk R2 (module-toggle dependents).
- **is_class_teacher semantics**: the spec body says `is_class_teacher?` on the assign endpoint. Interpreted as: if true AND section_id is provided, ALSO set Section.teacher_id to this teacher (in addition to creating the TeacherAssignment row). The TeacherAssignment model itself doesn't have an `is_class_teacher` flag — the canonical class-teacher designation lives on the Section row (one teacher per section). GET /teachers/:id/assignments computes `is_class_teacher` per-assignment by checking `section.teacher_id === teacher.id` so the frontend can render a "Class Teacher" badge without a separate query.
- **Lint**: 0 errors, 55 warnings — ALL pre-existing (none in B4.3 code). Confirmed via lint output filtering.
- **Curl**: all 12 endpoints return 401 JSON for unauthenticated requests (matches middleware gate + branches/roles/audit pattern). Authenticated smoke-test requires Postgres to be running (currently down in sandbox).
- **Notes for the next session (B4.4 / Employee API or B5.x Academic)**:
  * The Guardian soft-delete cascade pattern (transaction-wrapped soft-delete of dependent junction rows) should be reused for any future endpoint that soft-deletes a parent record with M:M junction rows — keeps the junction table consistent so future queries don't surface orphan links.
  * The `withAnyPermission` HOF was used here for the first time (Guardian endpoints) — this is the canonical pattern for endpoints reachable by both staff (`guardians.view`) AND scoped-self (`guardians.view.own`) users. Use it for any future endpoint with similar dual-scope semantics (e.g. `fees.view` + `fees.view.own` for guardian-portal fee endpoints, `attendance.view` + `attendance.view.own` for guardian-portal attendance endpoints).
  * The duplicate-active-assignment 409 structured body (`details: { existing_assignment_id, ... }`) lets the frontend render the inline AlertCard without re-fetching — if a future B5 academic endpoint has similar "duplicate active X" semantics (e.g. duplicate exam schedule), reuse this structured-409 pattern.
  * The Teacher POST requires a `user_id` (Teacher ↔ User is 1:1). When the B4.4 Employee API is built, consider adding a "create User + Teacher in one transaction" endpoint to simplify the onboarding flow (current API requires the User to be created first via a separate endpoint, then linked).
  * `academic_year` defaults to current year server-side (`new Date().getFullYear()`) on POST /teachers/assign. If a future session adds an AcademicYear model with a "current year" concept, replace this default with a lookup against that table.

---
Task ID: B4.2
Agent: full-stack-developer
Task: Build B4.2 — Admission API (Kanban + pipeline + register). Created 4 route files (6 endpoints) + 1 shared helper + appended 4 Zod schemas. Implements the full admission pipeline: list/create applications, view/update single records, move between Kanban stages (applied → interviewed → approved → registered / rejected), and convert approved applications into fully-enrolled Students with auto-provisioned Guardian + User accounts + FeePlan (3 installments).

Work Log:
- Read worklog.md (B0-B3 complete, B4.1 + B4.3 + B4.4 done by sibling agents) + key pattern files: branches/route.ts (GET/POST), branches/[id]/route.ts (params+withPermission), audit/route.ts (paginated list with filters), students/route.ts (B4.1 sibling — student code generation pattern, StudentGuardian nested write, StudentHistory bootstrap), employees/route.ts (B4.4 sibling — User provisioning with temp password + db.$transaction pattern), with-permission.ts (withPermission HOF + RouteContext), with-tenant.ts (getTenantContext + tenantWhere), api/helpers.ts (paginatedResponse + parsePagination), validation/schemas.ts (existing Zod patterns), prisma/schema.prisma (Admission model lines 910-950, Student 706-762, Guardian 766-797, StudentGuardian 801-820, FeePlan 1350-1380, FeeInstallment 1383-1419, Class 635-666, User 329-413, Role 419-446).
- Confirmed AdmissionStatus enum (applied/interviewed/approved/registered/rejected) + permission codes admission.view + admission.approve + admission.reject (in permissions.ts).
- Appended 4 Zod schemas to `src/lib/validation/schemas.ts` (admissions section, after `toggleModuleSchema` and before the existing Student B4.1 block — no existing schemas touched):
  * `createAdmissionSchema` — `{ applicant_name, applicant_name_bn?, phone, email?, desired_class_id (UUID), previous_education? (z.record(z.string(), z.unknown()) — Zod 4 two-arg form), guardian_name, guardian_phone }`
  * `updateAdmissionSchema` — partial of create + `notes?`; allows clearing email via empty string literal
  * `updateAdmissionStatusSchema` — `{ status: enum[interviewed/approved/registered/rejected], rejection_reason? }` (excludes `applied` from the enum since you can't move TO applied — it's the default on POST)
  * `registerStudentSchema` — `{ gender?, dob? (ISO 8601), roll? }` — all optional overrides for the conversion
- Created `src/lib/admissions/register.ts` (NEW shared helper file — source of truth for admission→student conversion):
  * `convertAdmissionToStudent(admissionId, orgId, branchId, decidedBy, options?)` returns `{ created, student_id, fee_plan_id, user_id, guardian_id, student_code, user_created, temp_password?, message }`
  * Idempotent: if `admission.student_id` is set, returns existing IDs without re-creating
  * Status guard: only `approved` can be moved to `registered` (throws "Cannot register admission with status: X. Must be approved first.")
  * Resolves `desired_class` (UUID string) against Class table
  * Resolves Guardian role for the org (needed to provision User)
  * Look up existing Guardian by `(org, parent_phone)` — `@@unique([organization_id, phone])` guarantees ≤1 match
  * Provision User account if Guardian has none: email = parent_email OR synthesized `guardian.{digits}@madrasha.local` (deterministic, satisfies UNIQUE constraint without requiring email from form); reuses existing User with same email if present; otherwise creates new User with role=guardian, password_hash from 12-char server-generated temp password (per SRS §6.2.3 R-S2), avatar_initial = first 2 chars of parent_name uppercased
  * Creates Guardian if it doesn't exist; else backfills `user_id` on existing row
  * Generates student code `MOS-{year}-{seq4}` (matches students/route.ts format — count-then-insert, zero-padded to 4 digits, count includes soft-deleted rows so codes never get reused)
  * Resolves roll: caller-supplied OR `MAX(roll) + 1` for the class
  * Resolves DOB: caller-supplied OR `today - 10 years` placeholder (staff updates via PATCH /students/:id)
  * Creates Student + StudentGuardian junction (is_primary=true, receive_sms=true, can_pickup=true) + StudentHistory bootstrap row (action="admitted" — Risk R4 timeline seed chip) in nested write
  * Creates FeePlan (default BDT 12,000 total, 3 installments of BDT 4,000 each, due dates spaced 4 months apart starting from registration month) with nested `fee_installments.create[]` — all atomic
  * Flips Admission.status → "registered", sets student_id, stamps decided_by + decided_at
  * All writes wrapped in `db.$transaction(async (tx) => { ... })` so any failure (Guardian/User/Student/StudentGuardian/StudentHistory/FeePlan/FeeInstallment/Admission-update) rolls back — no orphan rows
- Created `src/app/api/v1/admissions/route.ts`:
  * GET (perm: admission.view) — paginated list (`parsePagination`) with filters: `status` (validated against AdmissionStatus enum — clean 400 on unknown values), `class_id` (exact string match on `desired_class`). Tenant-scoped via `tenantWhere(ctx)`. Returns `paginatedResponse` with remapped body field names (`parent_name → guardian_name`, `parent_phone → phone`, `parent_email → email`) so the frontend contract matches the create/update body shape.
  * POST (perm: admission.view — any staff can create an application). Validates `createAdmissionSchema`. Pre-flight 404 check on `desired_class_id` against Class table. Maps body fields to schema columns (`phone → parent_phone`, `email → parent_email`, `guardian_name → parent_name`, `guardian_phone → parent_phone`). Stores class UUID as string in `desired_class` (the schema models it as a free-form String, not a UUID FK — keeps historical applications intact if the class is later renamed or deleted). Sets `status="applied"`, `form_source="office"`, `requested_by=current_user`, `decided_by=null`. Returns new row + `desired_class_name` so the frontend Kanban card can show the class label without a second round-trip.
- Created `src/app/api/v1/admissions/[id]/route.ts`:
  * GET (perm: admission.view) — single record with all fields + joined `requester`, `decider`, and (if registered) the linked `student` summary. 404 if not found in tenant.
  * PATCH (perm: admission.view) — validates `updateAdmissionSchema` (partial). Pre-flight: 404 if not in tenant, 409 if status is `registered` (after registration the Student record is the source of truth — edits must go through PATCH /students/:id instead). If `desired_class_id` changes, verifies new class exists in org. Builds `Prisma.AdmissionUpdateInput` mapping body → schema column names; supports clearing email (empty string → null). Sets `updated_by = ctx.user_id`.
- Created `src/app/api/v1/admissions/[id]/status/route.ts`:
  * PATCH (perm: admission.approve — covers interview, approve, register, reject). Validates `updateAdmissionStatusSchema`. Enforces the linear pipeline via a `TRANSITIONS` map:
    ```
    ...
    ```
    Returns 409 with `{ current_status, attempted, allowed }` for invalid transitions; 409 with `{ current_status }` for terminal states. Requires `rejection_reason` (non-empty) when status === "rejected" (400 otherwise).
  * For status === "registered", delegates to the shared `convertAdmissionToStudent()` helper. The response then carries `{ student_id, fee_plan_id, user_id, guardian_id, student_code, user_created, temp_password? }`. The temp_password is included one-time-only when a NEW User was provisioned (matches the B4.4 employees POST pattern; per SRS §6.2.3 R-S3 — never logged, never persisted in plaintext, never returned by any other endpoint).
  * For status === "rejected", sets `rejection_reason` and stamps `decided_by` + `decided_at`. For interview/approve transitions, clears any stale `rejection_reason`.
  * Error-mapping layer translates the helper's thrown errors to HTTP status codes: "Admission not found" → 404, "Admission has no desired_class set" → 422, "Desired class not found" → 404, "Guardian role not configured" → 422, "Cannot register admission with status" → 409, P2002 → 409, unknown → 500.
- Created `src/app/api/v1/admissions/[id]/register/route.ts`:
  * POST (perm: admission.approve). Optional body `registerStudentSchema` (`{ gender?, dob?, roll? }` — all optional with sensible defaults). Pre-flight 404 check on the admission (clean error path before entering the transaction). Delegates to `convertAdmissionToStudent()`. Returns `{ admission_id, student_id, fee_plan_id, user_id, guardian_id, student_code, user_created, temp_password? }`. Idempotent: re-POSTing returns the existing IDs without re-creating anything.
  * Same error-mapping layer as /status endpoint for the registration path.
- Lint verification: `bun run lint` → **0 errors, 55 warnings** (ALL pre-existing in `src/components/ui/*` shadcn files + `src/components/dev/*` + `src/components/shell/TopBar.tsx` + `src/components/finance/CollectPaymentDialog.tsx` — NONE in the new files). Confirmed via `bun run lint 2>&1 | grep -E "admissions|lib/admissions|schemas"` → empty.
- TypeScript verification (`npx tsc --noEmit`): 3 errors in my new files — ALL pre-existing RequestHandler variance pattern matches:
  * `admissions/[id]/route.ts(115,3)` + `admissions/[id]/status/route.ts(68,3)` + `admissions/[id]/register/route.ts(70,3)` — `Promise<{ id: string }>` not assignable to `Promise<Record<string, string>>` because Promises are invariant. This is the EXACT same pattern as `branches/[id]/route.ts(76,3)` + `(126,3)`, `audit/[id]/route.ts(49,3)`, `employees/[id]/route.ts(160,3)` + `(386,3)`. Verified by running tsc against those files — they produce the same error. Since the spec forbids modifying `with-permission.ts`, I matched the established convention rather than working around it. Runtime behavior is unaffected — the handler still receives `{ params: Promise<{ id: string }> }` and correctly `await`s it.
  * Two TS errors found and FIXED during development: (1) `route.ts(93,37)` — used `Prisma.AdmissionStatus` (doesn't exist on the Prisma namespace). Fixed by importing `AdmissionStatus` type directly from `@/generated/prisma` and casting `status as AdmissionStatus`. (2) `[id]/route.ts(216,31)` — used `{ connect: { id: ... } }` on the `updated_by` field which is a plain `String? @db.Uuid`, not a relation. Fixed by setting the value directly: `updateData.updated_by = tenantCtx.user_id;`.
- Curl verification against the running dev server on port 3000 (no session cookie → middleware returns 401 JSON):
  * `GET /api/v1/admissions` → 401 `{"error":"Unauthorized","message":"Authentication required. Sign in at /login."}` (gate ✓)
  * `POST /api/v1/admissions` → 401 (gate ✓)
  * `GET /api/v1/admissions/abc` → 401 (gate ✓)
  * `PATCH /api/v1/admissions/abc` → 401 (gate ✓)
  * `PATCH /api/v1/admissions/abc/status` → 401 (gate ✓)
  * `POST /api/v1/admissions/abc/register` → 401 (gate ✓)
  * `GET /api/v1/branches` → 401 (control — matches established pattern ✓)
- End-to-end DB tests couldn't be run because PostgreSQL isn't running in this sandbox. To smoke-test end-to-end: start Postgres (`bun run db:up`), run `bunx prisma db push && bunx prisma db seed`, log in at /login as `administrator@madrasha.local` / `password123`, then:
  1. POST /api/v1/admissions with `{ applicant_name, phone, desired_class_id, guardian_name, guardian_phone }` → returns `status: "applied"`.
  2. PATCH /api/v1/admissions/:id/status with `{"status":"interviewed"}` → returns `decided_at` timestamp.
  3. PATCH /api/v1/admissions/:id/status with `{"status":"approved"}` → returns `status: "approved"`.
  4. POST /api/v1/admissions/:id/register → returns `{ student_id, fee_plan_id, user_id, student_code: "MOS-2026-XXXX", temp_password: "Hk9mQp2tRwXc" }`.
  5. Verify in DB: 1 new Student + 1 new Guardian + 1 new User (or existing reused) + 1 new StudentGuardian + 1 new StudentHistory + 1 new FeePlan + 3 new FeeInstallments + Admission.status === "registered" + Admission.student_id set.

Stage Summary:
- **Files created (5)**:
  1. `src/lib/admissions/register.ts` (shared helper — source of truth for admission→student conversion; idempotent; atomic via db.$transaction)
  2. `src/app/api/v1/admissions/route.ts` (GET list + POST create)
  3. `src/app/api/v1/admissions/[id]/route.ts` (GET single + PATCH update)
  4. `src/app/api/v1/admissions/[id]/status/route.ts` (PATCH status move with transition validation)
  5. `src/app/api/v1/admissions/[id]/register/route.ts` (POST convert to student)
- **Files modified (1, append-only)**:
  6. `src/lib/validation/schemas.ts` — appended `createAdmissionSchema`, `updateAdmissionSchema`, `updateAdmissionStatusSchema`, `registerStudentSchema` after `toggleModuleSchema`. Pre-existing schemas untouched.
- **Endpoints (7 total)**: GET/POST /admissions, GET/PATCH /admissions/:id, PATCH /admissions/:id/status, POST /admissions/:id/register
- **Permissions enforced**: admission.view (GET list, POST create, GET/PATCH single), admission.approve (PATCH status, POST register). The POST /admissions uses admission.view (any staff who can see applications can create them — front-desk walk-ins, office form submissions).
- **Pipeline transitions** (AdmissionStatus enum): applied → interviewed | rejected; interviewed → approved | rejected; approved → registered | rejected; registered / rejected = terminal. Status endpoint validates transitions via TRANSITIONS map; rejects invalid moves with 409 + structured body `{ current_status, attempted, allowed }`.
- **Registration atomicity**: helper wraps all writes (Guardian lookup/create, User provision, Student create, StudentGuardian junction, StudentHistory bootstrap, FeePlan create, FeeInstallment create × 3, Admission status update) in a single `db.$transaction` — failure in any step rolls back the entire batch.
- **Idempotency**: re-calling /register or /status(→registered) on an already-registered admission returns the existing student_id/fee_plan_id/user_id without re-creating anything. The temp_password is only surfaced when a NEW User was provisioned.
- **Student code**: `MOS-{year}-{seq4}` (e.g. `MOS-2026-0042`) — matches the format used by POST /api/v1/students (B4.1) so admissions and manual student entries share one continuous sequence per year.
- **Lint**: 0 errors. 55 warnings all pre-existing in shadcn/ui components + dev pages (raw px values in className — design-QA debt, not B4.2 concern).
- **Curl**: all 7 endpoints return 401 JSON for unauthenticated requests (matches middleware gate + branches pattern). Authenticated smoke-test requires Postgres to be running.
- **TS strictness**: 3 tsc errors in new files — ALL match the pre-existing `branches/[id]/route.ts` RequestHandler variance pattern (Promises are invariant). No new TS patterns introduced.
- **Notes for next session (B4.3 frontend Kanban)**:
  * The frontend Kanban board should call PATCH /api/v1/admissions/:id/status with `{"status":"registered"}` (NOT POST /register) when a user drags a card to the "Registered" column — the status endpoint delegates to the same shared helper and returns the same payload shape (with student_id, fee_plan_id, temp_password if user_created). The /register endpoint is provided as an explicit alternative for the "Register Now" button on a detail page (same payload, same logic — just a clearer URL for the action).
  * The temp_password returned is a ONE-TIME view. The frontend should display it in a modal that the staff member must communicate to the guardian out-of-band (phone call, SMS, etc.). Per SRS §6.2.3 R-S3: never logged, never persisted in plaintext, never returned by any other endpoint.
  * The `desired_class` field on Admission is a free-form String (not a UUID FK). The API stores the class UUID as a string — if the frontend Kanban card needs the human-readable class name, the POST /admissions response includes `desired_class_name` (resolved server-side), and the GET list endpoint does NOT include it (perf concern for large lists). Frontend should either join through a `/classes` lookup OR rely on the detail GET /admissions/:id endpoint to fetch the class name.
  * The default FeePlan amount (BDT 12,000 / 3 installments of BDT 4,000) is a placeholder. The fee clerk should adjust the actual amounts via PATCH /fee-plans/:id (out of scope for B4.2) based on the class's published fee structure.
  * The default DOB placeholder (today - 10 years) and default gender ("male") are intentionally minimal — staff MUST update these via PATCH /students/:id before the student record is considered complete. The frontend should flag students with placeholder DOBs (e.g. born on Jan 1 of `{currentYear - 10}`) for follow-up.
