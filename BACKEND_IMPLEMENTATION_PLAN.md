...
| B4 | 4.1 | Student API (CRUD + promotion) | ✅ Done | [`4521217`](https://github.com/sajidchowdhury/MadrashaOS/commit/4521217) | 2026-09-18 | `GET/POST /api/v1/students` + `GET/PATCH/DELETE /api/v1/students/[id]` + `POST /api/v1/students/[id]/promote` (Risk R4: creates StudentHistory) + `GET /api/v1/students/[id]/history` (timeline) |
| B4 | 4.2 | Admission API (Kanban + pipeline) | ✅ Done | [`4521217`](https://github.com/sajidchowdhury/MadrashaOS/commit/4521217) | 2026-09-18 | `GET/POST /api/v1/admissions` + `GET/PATCH /api/v1/admissions/[id]` + `PATCH /api/v1/admissions/[id]/status` (stage transitions) + `POST /api/v1/admissions/[id]/register` (auto-creates Student + FeePlan + User) |
| B4 | 4.3 | Guardian + Teacher API | ✅ Done | [`4521217`](https://github.com/sajidchowdhury/MadrashaOS/commit/4521217) | 2026-09-18 | `GET/POST /api/v1/guardians` + `GET/PATCH/DELETE /api/v1/guardians/[id]` + `GET /api/v1/guardians/[id]/children` + `GET/POST /api/v1/teachers` + `GET/PATCH /api/v1/teachers/[id]` + `POST /api/v1/teachers/assign` (409 on duplicate) + `GET /api/v1/teachers/[id]/assignments` |
| B4 | 4.4 | Employee API | ✅ Done | [`4521217`](https://github.com/sajidchowdhury/MadrashaOS/commit/4521217) | 2026-09-18 | `GET/POST /api/v1/employees` + `GET/PATCH/DELETE /api/v1/employees/[id]` (DELETE disables User account per SRS §2.2.6) |
| B5 | 5.1 | Academic Structure API (routine/calendar) | ✅ Done | [`4521217`](https://github.com/sajidchowdhury/MadrashaOS/commit/4521217) | 2026-09-18 | `GET/POST /api/v1/classes` + `GET/PATCH/DELETE /api/v1/classes/[id]` + `GET/POST /api/v1/classes/[id]/sections` + `GET/POST /api/v1/classes/[id]/routine` (409 on double-booked teacher) + `GET/POST /api/v1/subjects` + `GET/PATCH /api/v1/subjects/[id]` |
| B5 | 5.2 | Attendance API (idempotent submit) | ✅ Done | [`4521217`](https://github.com/sajidchowdhury/MadrashaOS/commit/4521217) | 2026-09-18 | `GET/POST /api/v1/attendance/sessions` (POST idempotent via Idempotency-Key header + existing session check; default present; audit log; Risk R6) + `GET/PATCH/DELETE /api/v1/attendance/sessions/[id]` (single with records; update blocked if locked; soft delete cascades to records) |
| B5 | 5.3 | Examination API (marks + publish) | ✅ Done | [`4521217`](https://github.com/sajidchowdhury/MadrashaOS/commit/4521217) | 2026-09-18 | `GET/POST /api/v1/exams` + `GET/PATCH/DELETE /api/v1/exams/[id]` (blocked if published) + `GET/PUT /api/v1/exams/[id]/marks` (validates mark > full_marks → 400) + `POST/DELETE /api/v1/exams/[id]/publish` (locks paper; audit logged) |
| B5 | 5.4 | Results API (mark sheet + GPA) | ✅ Done | [`4521217`](https://github.com/sajidchowdhury/MadrashaOS/commit/4521217) | 2026-09-18 | `GET /api/v1/results` (list with Risk R7 conditional position) + `GET /api/v1/results/[studentId]` (full mark sheet with per-subject breakdown) + `POST /api/v1/results/generate` (computes GPA + grade + division + position; blocked if exam not published; audit logged) |
| B6 | 6.1 | Fees API (collect payment + receipt) | ✅ Done | [`4521217`](https://github.com/sajidchowdhury/MadrashaOS/commit/4521217) | 2026-09-18 | `GET/POST /api/v1/fees/plans` + `GET/PATCH /api/v1/fees/plans/[id]` (with installments + payments + summary) + `GET/POST /api/v1/fees/payments` (Golden Flow: payment + balanced ledger + installment update + account balance update + audit log + idempotency) + `GET /api/v1/fees/payments/[id]` (receipt view) + `GET /api/v1/fees/outstanding` (Risk R12: "as of [timestamp]" + per-student breakdown) |
| B6 | 6.2 | Scholarship & Discount API | ✅ Done | [`4521217`](https://github.com/sajidchowdhury/MadrashaOS/commit/4521217) | 2026-09-18 | `GET/POST /api/v1/scholarships` (Risk R8: ≥50% or ≥৳10K auto-routes to approval queue; <threshold auto-approves + updates fee plan) + `GET/PATCH/DELETE /api/v1/scholarships/[id]` (revoke reverses fee plan) + `POST /api/v1/scholarships/[id]/approve` (approve/reject; approve updates fee plan; audit logged) |
| B6 | 6.3 | Accounting (GL) API (balanced entries) | ✅ Done | [`4521217`](https://github.com/sajidchowdhury/MadrashaOS/commit/4521217) | 2026-09-18 | `GET/POST /api/v1/accounts` (chart of accounts with fund/type filter) + `GET/POST /api/v1/ledger` (list with running balance; POST validates debit≠credit + fund isolation C6/D18 + ≥৳25K routes to pending) + `GET/PATCH /api/v1/ledger/[id]` (single; PATCH approve/reject updates balances) + `GET /api/v1/ledger/statement` (opening/closing balance + chronological entries) |
| B6 | 6.4 | Cash & Bank API | ✅ Done | [`4521217`](https://github.com/sajidchowdhury/MadrashaOS/commit/4521217) | 2026-09-18 | `POST /api/v1/cashbank/transfer` (both legs in one balanced ledger entry + balance check + fund isolation + audit) + `GET /api/v1/cashbank/transfers` (list with filters) + `GET /api/v1/cashbank/transfers/[id]` (single with linked ledger entry) |
| B6 | 6.5 | Zakat API (fund isolation) | ✅ Done | [`4521217`](https://github.com/sajidchowdhury/MadrashaOS/commit/4521217) | 2026-09-18 | `GET /api/v1/zakat` (fund-isolated dashboard with Risk R9: Zakat badge + non-Zakat greyed) + `POST /api/v1/zakat/receive` (posts to Zakat fund only; balanced ledger fund='zakat') + `POST /api/v1/zakat/distribute` (Risk R9: validates amount ≤ balance → 400 with fund badge) |
| B6 | 6.6 | Donations API (honeypot + receipt) | ✅ Done | [`4521217`](https://github.com/sajidchowdhury/MadrashaOS/commit/4521217) | 2026-09-18 | `GET/POST /api/v1/donations` (Risk R10: honeypot field silently rejects; email/mobile mandatory; public access via middleware bypass; Zakat donations post to Zakat fund) + `GET /api/v1/donations/[id]` (receipt view — public access by ID for receipt verification) |
| B7 | 7.1 | Inventory + Purchase API | ✅ Done | [`4521217`](https://github.com/sajidchowdhury/MadrashaOS/commit/4521217) | 2026-09-18 | `GET/POST /api/v1/inventory` + `GET/PATCH/DELETE /api/v1/inventory/[id]` + `POST /api/v1/inventory/receive` (stock increase + audit) + `POST /api/v1/inventory/issue` (validates qty ≤ stock → 400) + `GET/POST /api/v1/purchases` (auto PO number + total calc) + `GET/PATCH/DELETE /api/v1/purchases/[id]` + `POST /api/v1/purchases/[id]/approve` (approve/reject) + `POST /api/v1/purchases/[id]/receive` (updates inventory + status) + `GET/POST /api/v1/suppliers` + `GET/PATCH/DELETE /api/v1/suppliers/[id]` (blocks if outstanding balance) |
| B7 | 7.2 | Supplier + Asset API | ✅ Done | [`4521217`](https://github.com/sajidchowdhury/MadrashaOS/commit/4521217) | 2026-09-18 | Suppliers already built in B7.1. Assets: `GET/POST /api/v1/assets` (summary with total_current_value + depreciation) + `GET/PATCH/DELETE /api/v1/assets/[id]` (single with is_disposed + record_retained; DELETE blocked if in-use) + `POST /api/v1/assets/[id]/transfer` (perm: assets.transfer; to different branch; audit) + `POST /api/v1/assets/[id]/dispose` (perm: assets.dispose; SRS §2.5.4 record kept; audit) |
| B7 | 7.3 | Hostel + Food/Meal API | ✅ Done | [`4521217`](https://github.com/sajidchowdhury/MadrashaOS/commit/4521217) | 2026-09-18 | `GET/POST /api/v1/hostel/rooms` (with occupancy summary + auto-create beds) + `POST /api/v1/hostel/beds/[id]/allocate` (409 if occupied; gender match check) + `POST /api/v1/hostel/beds/[id]/deallocate` + `GET/POST /api/v1/food/meal-plans` (weekly cost summary) + `POST /api/v1/food/expense` (Golden Flow: posts balanced ledger to Operating Expenses) |
| B7 | 7.4 | Library + Transport API | ✅ Done | [`4521217`](https://github.com/sajidchowdhury/MadrashaOS/commit/4521217) | 2026-09-18 | `GET/POST /api/v1/library/books` + `POST /api/v1/library/issue` (409 if no copies available) + `POST /api/v1/library/return` (increments available_copies) + `GET/POST /api/v1/transport/vehicles` (with insurance/fitness expiry alerts) + `GET/POST /api/v1/transport/fuel` (Golden Flow: balanced ledger to Operating Expenses; updates vehicle value on maintenance) |
| B8 | 8.1 | Notices API (audience + recipient count) | ✅ Done | [`4521217`](https://github.com/sajidchowdhury/MadrashaOS/commit/4521217) | 2026-09-18 | `GET/POST /api/v1/notices` (Risk R11: computes recipient_count by audience + filter; checks notices.send perm for status=sent) + `GET/PATCH/DELETE /api/v1/notices/[id]` + `POST /api/v1/notices/preview-recipients` (Risk R11: live recipient list before send) |
...
**Summary:** 34 / 36 sessions done · 0 in progress · 2 pending · 0 blocked
