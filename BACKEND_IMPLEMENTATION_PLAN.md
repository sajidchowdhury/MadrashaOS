...
| B4 | 4.1 | Student API (CRUD + promotion) | ✅ Done | [`b03d5cc`](https://github.com/sajidchowdhury/MadrashaOS/commit/b03d5cc) | 2026-09-18 | `GET/POST /api/v1/students` + `GET/PATCH/DELETE /api/v1/students/[id]` + `POST /api/v1/students/[id]/promote` (Risk R4: creates StudentHistory) + `GET /api/v1/students/[id]/history` (timeline) |
| B4 | 4.2 | Admission API (Kanban + pipeline) | ✅ Done | [`b03d5cc`](https://github.com/sajidchowdhury/MadrashaOS/commit/b03d5cc) | 2026-09-18 | `GET/POST /api/v1/admissions` + `GET/PATCH /api/v1/admissions/[id]` + `PATCH /api/v1/admissions/[id]/status` (stage transitions) + `POST /api/v1/admissions/[id]/register` (auto-creates Student + FeePlan + User) |
| B4 | 4.3 | Guardian + Teacher API | ✅ Done | [`b03d5cc`](https://github.com/sajidchowdhury/MadrashaOS/commit/b03d5cc) | 2026-09-18 | `GET/POST /api/v1/guardians` + `GET/PATCH/DELETE /api/v1/guardians/[id]` + `GET /api/v1/guardians/[id]/children` + `GET/POST /api/v1/teachers` + `GET/PATCH /api/v1/teachers/[id]` + `POST /api/v1/teachers/assign` (409 on duplicate) + `GET /api/v1/teachers/[id]/assignments` |
| B4 | 4.4 | Employee API | ✅ Done | [`b03d5cc`](https://github.com/sajidchowdhury/MadrashaOS/commit/b03d5cc) | 2026-09-18 | `GET/POST /api/v1/employees` + `GET/PATCH/DELETE /api/v1/employees/[id]` (DELETE disables User account per SRS §2.2.6) |
| B5 | 5.1 | Academic Structure API (routine/calendar) | ✅ Done | [`b03d5cc`](https://github.com/sajidchowdhury/MadrashaOS/commit/b03d5cc) | 2026-09-18 | `GET/POST /api/v1/classes` + `GET/PATCH/DELETE /api/v1/classes/[id]` + `GET/POST /api/v1/classes/[id]/sections` + `GET/POST /api/v1/classes/[id]/routine` (409 on double-booked teacher) + `GET/POST /api/v1/subjects` + `GET/PATCH /api/v1/subjects/[id]` |
| B5 | 5.2 | Attendance API (idempotent submit) | ✅ Done | [`b03d5cc`](https://github.com/sajidchowdhury/MadrashaOS/commit/b03d5cc) | 2026-09-18 | `GET/POST /api/v1/attendance/sessions` (POST idempotent via Idempotency-Key header + existing session check; default present; audit log; Risk R6) + `GET/PATCH/DELETE /api/v1/attendance/sessions/[id]` (single with records; update blocked if locked; soft delete cascades to records) |
| B5 | 5.3 | Examination API (marks + publish) | ✅ Done | [`b03d5cc`](https://github.com/sajidchowdhury/MadrashaOS/commit/b03d5cc) | 2026-09-18 | `GET/POST /api/v1/exams` + `GET/PATCH/DELETE /api/v1/exams/[id]` (blocked if published) + `GET/PUT /api/v1/exams/[id]/marks` (validates mark > full_marks → 400) + `POST/DELETE /api/v1/exams/[id]/publish` (locks paper; audit logged) |
| B5 | 5.4 | Results API (mark sheet + GPA) | ✅ Done | [`b03d5cc`](https://github.com/sajidchowdhury/MadrashaOS/commit/b03d5cc) | 2026-09-18 | `GET /api/v1/results` (list with Risk R7 conditional position) + `GET /api/v1/results/[studentId]` (full mark sheet with per-subject breakdown) + `POST /api/v1/results/generate` (computes GPA + grade + division + position; blocked if exam not published; audit logged) |
...
**Summary:** 23 / 36 sessions done · 0 in progress · 13 pending · 0 blocked
